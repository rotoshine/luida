        ...
        <Window theme={theme} title="상태 뱃지">
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {['running', 'reviewing', 'needs_approval', 'pr_ready', 'completed', 'failed', 'aborted', 'pending'].map((k) => (
              <Badge key={k} theme={theme} kind={k} label={window.LUIDA_I18N[lang].statusBadges[k]} />
            ))}
          </div>
          <div style={{ height: 10 }} />
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {['busy', 'idle', 'offline'].map((k) => (
              <Badge key={k} theme={theme} kind={k} label={window.LUIDA_I18N[lang].statusBadgesAdv[k]} />
            ))}
          </div>
        </Window>
...
Object.assign(window, { MoodboardTab, TokensTab, ComponentsTab, SectionTitle });
